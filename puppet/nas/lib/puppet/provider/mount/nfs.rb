...
Puppet::Type.type(:mount).provide(
  ...
  Mount_timeout = 180 # puppet will retry to mount if failure happens for this period of time
  Try_sleep = 5 # sleep time between retries.
  ...
  def self.prefetch_hook(target_records)
    target_records.collect do |record|
      record[:ensure] = :unmounted if record[:record_type] == :nfs
      record
    end
  end
  ...
  def mount
    ...
    begin
      ...
      if except_message.include? "already mounted"
        end
      Puppet.warning("Mount attempt for #{resource[:name]} failed, retry in #{Try_sleep}s")
      if Time.now < start + Mount_timeout
        retry
      else
        raise
      end
